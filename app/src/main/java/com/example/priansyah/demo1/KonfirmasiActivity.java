...
//                intent.putExtra("LIST", listOfItemsKfm);
                ...
                dbTransaction.execSQL("Create table if not exists stock_transaction(transaction_id INT, total INT, discount INT, tax VARCHAR, date_created VARCHAR);");
...
//
                for (int i = 1; i < listOfTransactionDetail.size() + 1; i++) {
                    ...
                    if(maxTransDetail==null) {
                        ...
                        if (maxTransDetail.moveToFirst())
                            dbTransaction.execSQL("insert into transaction_detail values('" + (maxTransDetail.getInt(0) + i) + "', '" + (maxTrans.getInt(0) +1) + "', '" + listOfTransactionDetail.get(i - 1).getTextSKU() + "', '" + listOfTransactionDetail.get(i - 1).getTextHarga() + "', '" + date + "')");
